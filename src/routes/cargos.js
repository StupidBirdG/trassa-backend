...
// GET /api/cargos
// shipper — видит свои грузы (все статусы)
// carrier — видит все открытые грузы + свои заявки
router.get('/', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(`
      SELECT c.*,
        u.name AS owner_name, u.phone AS owner_phone, u.company_name AS owner_company,
    ...
    if (rows.length) {
      const { rows: bidRows } = await pool.query(`
        SELECT b.*, u.name AS carrier_name, u.phone AS carrier_phone, u.company_name
...
router.post('/', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      'INSERT INTO cargos (owner_id, from_city, to_city, weight_tons, cargo_type, pickup_date, price, comment) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *',
