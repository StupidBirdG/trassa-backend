...
router.get('/:cargoId', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query("SELECT m.id, m.text, m.created_at, m.sender_id, u.name AS sender_name FROM messages m JOIN users u ON u.id = m.sender_id WHERE m.cargo_id = $1 ORDER BY m.created_at ASC", [req.params.cargoId]);
    res.json(rows);
...
router.post('/:cargoId', async (req, res) => {
  try {
    ...
    if (!(await checkAccess(req.params.cargoId, req.user.id))) return res.status(403).json({ error: 'No access' });
    const { rows } = await pool.query('INSERT INTO messages (cargo_id, sender_id, text) VALUES ($1,$2,$3) RETURNING *', [req.params.cargoId, req.user.id, text.trim()]);
